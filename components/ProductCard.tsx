    ...
    <Link href={`/products/${product.id}`} className="block h-full">
      <div className="bg-white border border-gray-200 rounded overflow-hidden hover:shadow-md transition-all duration-200 flex flex-col h-full">
        ...
        <div className="relative aspect-square bg-white flex-shrink-0">
          ...
          {product.discountPercent > 0 && (
            <div className="absolute top-1.5 sm:top-2 left-1.5 sm:left-2">
              <span className="bg-red-500 text-white text-[9px] sm:text-[10px] md:text-xs font-semibold px-1 sm:px-1.5 md:px-2 py-0.5 rounded">
          ...
          <button
            ...
            className="absolute top-1.5 sm:top-2 right-1.5 sm:right-2 p-1 sm:p-1.5 bg-white/90 backdrop-blur-sm rounded-full shadow-sm hover:bg-white transition-colors z-10"
            aria-label="Add to favorites"
          >
            <Heart size={14} className="sm:w-4 sm:h-4 text-gray-600" />
          </button>
        </div>
        ...
        <div className="p-2 sm:p-2.5 md:p-3 flex-1 flex flex-col">
          ...
          <h3 className="text-[11px] sm:text-xs md:text-sm font-medium text-gray-900 mb-1 sm:mb-1.5 line-clamp-2 min-h-[2rem] sm:min-h-[2.5rem] leading-tight sm:leading-snug">
            {product.name}
          </h3>

          {/* Brand - Matching agribegri.com style */}
          {product.brand && (
            <p className="text-[9px] sm:text-[10px] md:text-xs text-gray-600 mb-1 sm:mb-1.5 line-clamp-1">
              {product.brand}
            </p>
          )}
          ...
          {product.rating !== null && product.reviews !== null && (
            <div className="flex items-center gap-0.5 sm:gap-1 mb-1.5 sm:mb-2">
              <div className="flex items-center">
                ...
                <span className="text-[10px] sm:text-xs text-gray-700 ml-0.5 font-medium">{product.rating}</span>
              </div>
          ...
          <div className="mb-1.5 sm:mb-2 flex items-baseline gap-1.5 sm:gap-2">
            {selectedVariant.price !== null ? (
              ...
              <span className="text-[10px] sm:text-xs md:text-sm text-gray-500">Price not available</span>
            )}
          </div>

          {/* Quantity - Matching agribegri.com style */}
          <p className="text-[9px] sm:text-[10px] md:text-xs text-gray-600 mb-2 sm:mb-3 line-clamp-1">
          ...
          <button
            ...
            className="w-full bg-[#16a34a] hover:bg-[#15803d] disabled:bg-gray-400 disabled:cursor-not-allowed text-white text-[10px] sm:text-xs md:text-sm font-medium py-1.5 sm:py-2 md:py-2.5 rounded transition-colors mt-auto flex items-center justify-center"
          >
