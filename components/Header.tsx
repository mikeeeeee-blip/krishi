  ...
  return (
    <header className="bg-white shadow-sm border-b border-gray-100 relative z-30 w-full">
      <div className="w-full px-3 sm:px-4 md:px-6">
        {/* Mobile Layout */}
        <div className="flex lg:hidden items-center justify-between py-3 gap-2 sm:gap-3">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-1.5 flex-shrink-0">
            <div className="text-lg sm:text-xl font-bold flex items-center">
              ...
              <span className="text-[#2563eb] relative">
                <span className="absolute -top-0.5 left-0 text-green-600 text-base sm:text-lg">🌿</span>
                <span className="relative z-10">Begri</span>
              </span>
            </div>
          </Link>

          {/* Search Bar - Improved sizing */}
          <div className="flex-1 flex items-center gap-1 sm:gap-1.5 mx-1 sm:mx-2 min-w-0">
            <div className="flex-1 relative min-w-0">
              <input
                type="text"
                placeholder="Search..."
                className="w-full px-3 sm:px-4 py-2 sm:py-2.5 border border-gray-300 rounded-l-md focus:outline-none focus:ring-2 focus:ring-green-500 text-sm sm:text-base h-[36px] sm:h-[40px]"
              />
            </div>
            <button className="bg-[#2563eb] text-white px-2.5 sm:px-3 py-0 rounded-r-md hover:bg-[#1d4ed8] flex items-center justify-center transition-colors h-[36px] sm:h-[40px] aspect-square flex-shrink-0">
              <Search size={16} className="sm:w-4 sm:h-4" />
            </button>
          </div>

          {/* User Actions - Icons Only with proper spacing */}
          <div className="flex items-center gap-1.5 sm:gap-2.5 flex-shrink-0">
            <Link href="/cart" className="relative text-gray-700 hover:text-green-600 transition-colors p-1.5 sm:p-2 flex items-center justify-center">
              <ShoppingCart size={20} className="sm:w-6 sm:h-6" />
              {cartCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 sm:-top-1 sm:-right-1 bg-red-500 text-white text-[10px] rounded-full w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center font-semibold">
                  {cartCount > 9 ? '9+' : cartCount}
                </span>
              )}
            </Link>
            <button className="text-gray-700 hover:text-green-600 transition-colors p-1.5 sm:p-2 flex items-center justify-center">
